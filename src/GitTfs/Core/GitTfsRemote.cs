...
namespace GitTfs.Core
{
    public class GitTfsRemote : IGitTfsRemote
    {
        ...
        public IFetchResult FetchWithMerge(int mergeChangesetId, bool stopOnFailMergeCommit = false, int lastChangesetIdToFetch = -1, IRenameResult renameResult = null, params string[] parentCommitsHashes)
        {
            ...
            do
            {
                ...
                foreach (var changeset in fetchedChangesets)
                {
                    ...
                    parentSha ??= renameResult?.IsProcessingRenameChangeset == true ? renameResult.LastParentCommitBeforeRename : MaxCommitHash;
                    ...
                    {
                        if (renameResult?.IsProcessingRenameChangeset != true)
                        {
        ...
        private string CommitTheGitIgnoreFile(string pathToGitIgnoreFile)
        {
        ...
        private bool ProcessBranchRenameChangeset(ITfsChangeset changeset, bool stopOnFailMergeCommit, ref string parentCommit)
        {
            ...
            var originalBranchName = Tfs.GetBranchNameBeforeRename(TfsRepositoryPath, changeset.Summary.ChangesetId);
